...
res_accumulated = load('KTH_EvaluationResults_2017-04-12-11-25.mat');
option_accumulated = load('KTH_option_2017-04-12-11-25.mat');

res_batch =load('KTH_EvaluationResults_2017-04-12-11-21.mat');
option_batch =load('KTH_option_2017-04-12-11-21.mat');

test_labels = res_accumulated.eval_res.svm.test_labels;
Ytt = [];
B_track = {};
...
labels_p_a = res_accumulated.eval_res.svm.prob_estimates(nnsb+1: nnsb+nn_snippets,:);
labels_p_normalized_a = labels_p_a./repmat(sum(labels_p_a,2), 1,size(labels_p_a,2));

labels_p_b = res_batch.eval_res.svm.prob_estimates(nnsb+1: nnsb+nn_snippets,:);
...
%         end
        ...
        b = max([labels_p_normalized_a(idxc,:);labels_p_normalized_b(idxc,:)] );
        b = b/sum(b);
        idxc = idxc+1;
...
save('KTH_mix.mat','reg_res_curve');
