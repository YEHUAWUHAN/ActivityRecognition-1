function [c,g,bestcv,acc,cmat,bestmodel] = Fu_libsvm_cv(Ktrain,train_label,opts)
% This function is to make cross validation and find the best model
%
% [c,g,bestcv,acc,cmat,bestmodel] = Fu_libsvm_cv(Ktrain,train_label,opts)
%
% Input:
...
if opts.customKernel==1
    warning('Using custom Kernel: You must set the kernel matrix in opts.K \n');
    if ~isfield(opts,'K') 
        error('You should set the kernel matrix in opts.K \n which will be used as final training data for SVM! \n');
    end
end
...
for i = 1 : opts.repeatFinalEst
    ...
    if(opts.customKernel)
        ...
        [lab1, acc1p,dec] = svmpredict(Y2, K1te, model); 
        model = libsvmtrain(Y2, K2tr, [cmd, ' -t 4 ']);
        [lab2, acc2p,dec] = svmpredict(Y1, K2te, model); 
    else
        cmd = [weightstr,' -c ' num2str(c) ' -g ' num2str(g)];
        model = libsvmtrain(Y1, X1, cmd);
        [lab1, acc1p ,dec] = svmpredict(Y2, X2, model); 
        model = libsvmtrain(Y2, X2, cmd);
        [lab2, acc2p,dec] = svmpredict(Y1, X1, model);
    end
